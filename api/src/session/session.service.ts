...
import { IServerConfig, ConfigService } from "../config/config.service.js";
...
export class SessionService {
  ...
  async refreshSession(sessionId: string) {
    return await this.redis.refreshExpire<CacheUser>(
      RedisKeys.SESSION,
      sessionId
    );
  }
  ...
  async setSession(sessionId: string, user: User) {
    try {
      ...
      const cacheUser: CacheUser = {
        ...user,
        sessionExpires: this.date
          .getDaysFromNow(sessionExpiresInDays)
          .toISOString(),
      };
      const sessionExpireInSeconds =
        this.date.secondsFromNumberOfDays(sessionExpiresInDays);
      await this.redis.setWithRetry(
        RedisKeys.SESSION,
        sessionId,
        cacheUser,
        4,
